...
local bUp, bDown, bX, bA = cm.input.getVirtualInput("Up") ,cm.input.getVirtualInput("Down"),cm.input.getVirtualInput("X"),cm.input.getVirtualInput("A")
local Font = cfg.font[1]

local function selectMenu(Title, Options)
    local layer2 = cm.layer.new()
    ...
    for i,text in ipairs(Options) do
        local sprite = cm.sprite.new(sw/2,y+lHeight*i+lHeight/2, Font, text)
    ...
    local title = cm.sprite.new(sw/2,y,Font,Title)
    title:center("middle","top")
    print("------------------")
    local w = title:getSize()
    width = math.max(w,width)
    local x = sw/2-width/2
    local shadow = cm.sprite.new(sw/2,sh/2,cm.sourceImage.new(iD))
    shadow:setSca(width,height)
    shadow:center("middle","middle")
    local coverBox = cm.sprite.new(0,0,box,{sw,sh})
    coverBox:setTint(0,0,0,0)
    local sbox2 = cm.sprite.new(x-20,y,box,{width+20,height})
    sbox2:setTint(unpack(Color1))
    local sbox1 = cm.sprite.new(x-20,y,box,{width+20,lHeight})
    ...
    cm.thread.waitThread(cam:movePosTo(0,0,0.5))
    local sfx = love.audio.newSource("frontend/sounds/two_tone_nav.wav", "static")
    --cm.thread.wait(0.25)
    while not choice do
        if index ~= oldindex then indexChange() end
        if bDown.isDown() then index = index < #Options and index+1 or 1 sfx:play() cm.thread.wait(0.25) end
        if bUp.isDown() then index = index > 1 and index-1 or #Options sfx:play() cm.thread.wait(0.25) end
    ...
    sfx2:play()
    cm.thread.waitThread(cam:movePosTo(0,-sh,0.5))
    cm.layer.remove(layer)
    cm.layer.remove(layer2)
    return choice
end
